IF object_id(N'[dbo].[FK_dbo.CustomerReserve_dbo.StaffBusinessItem_StaffBusinessItemId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [FK_dbo.CustomerReserve_dbo.StaffBusinessItem_StaffBusinessItemId]
IF object_id(N'[dbo].[FK_dbo.CustomerReserve_dbo.StoreDetail_StoreId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [FK_dbo.CustomerReserve_dbo.StoreDetail_StoreId]
IF object_id(N'[dbo].[FK_dbo.StaffBusinessItem_dbo.StoreDetail_StoreId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [FK_dbo.StaffBusinessItem_dbo.StoreDetail_StoreId]
IF object_id(N'[dbo].[FK_dbo.StaffBusinessItem_dbo.BusinessItem_BusinessItemId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [FK_dbo.StaffBusinessItem_dbo.BusinessItem_BusinessItemId]
IF object_id(N'[dbo].[FK_dbo.StaffBusinessItem_dbo.StaffDetail_StaffId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [FK_dbo.StaffBusinessItem_dbo.StaffDetail_StaffId]
IF object_id(N'[dbo].[FK_dbo.CustomerReserve_dbo.CustomerDetail_CustomerId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [FK_dbo.CustomerReserve_dbo.CustomerDetail_CustomerId]
IF object_id(N'[dbo].[FK_dbo.CustomerReserve_dbo.StaffDetail_StaffId]', N'F') IS NOT NULL
    ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [FK_dbo.CustomerReserve_dbo.StaffDetail_StaffId]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_BusinessItemId' AND object_id = object_id(N'[dbo].[StaffBusinessItem]', N'U'))
    DROP INDEX [IX_BusinessItemId] ON [dbo].[StaffBusinessItem]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_StoreId' AND object_id = object_id(N'[dbo].[StaffBusinessItem]', N'U'))
    DROP INDEX [IX_StoreId] ON [dbo].[StaffBusinessItem]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_StaffId' AND object_id = object_id(N'[dbo].[StaffBusinessItem]', N'U'))
    DROP INDEX [IX_StaffId] ON [dbo].[StaffBusinessItem]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_StoreId' AND object_id = object_id(N'[dbo].[CustomerReserve]', N'U'))
    DROP INDEX [IX_StoreId] ON [dbo].[CustomerReserve]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_StaffId' AND object_id = object_id(N'[dbo].[CustomerReserve]', N'U'))
    DROP INDEX [IX_StaffId] ON [dbo].[CustomerReserve]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_CustomerId' AND object_id = object_id(N'[dbo].[CustomerReserve]', N'U'))
    DROP INDEX [IX_CustomerId] ON [dbo].[CustomerReserve]
IF EXISTS (SELECT name FROM sys.indexes WHERE name = N'IX_StaffBusinessItemId' AND object_id = object_id(N'[dbo].[CustomerReserve]', N'U'))
    DROP INDEX [IX_StaffBusinessItemId] ON [dbo].[CustomerReserve]
DECLARE @var0 nvarchar(128)
SELECT @var0 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.StaffBusinessItem')
AND col_name(parent_object_id, parent_column_id) = 'BusinessItemId';
IF @var0 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [' + @var0 + ']')
ALTER TABLE [dbo].[StaffBusinessItem] ALTER COLUMN [BusinessItemId] [int] NOT NULL
DECLARE @var1 nvarchar(128)
SELECT @var1 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.StaffBusinessItem')
AND col_name(parent_object_id, parent_column_id) = 'StaffId';
IF @var1 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [' + @var1 + ']')
ALTER TABLE [dbo].[StaffBusinessItem] ALTER COLUMN [StaffId] [int] NOT NULL
DECLARE @var2 nvarchar(128)
SELECT @var2 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.CustomerReserve')
AND col_name(parent_object_id, parent_column_id) = 'StaffId';
IF @var2 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [' + @var2 + ']')
ALTER TABLE [dbo].[CustomerReserve] ALTER COLUMN [StaffId] [int] NOT NULL
DECLARE @var3 nvarchar(128)
SELECT @var3 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.CustomerReserve')
AND col_name(parent_object_id, parent_column_id) = 'CustomerId';
IF @var3 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [' + @var3 + ']')
ALTER TABLE [dbo].[CustomerReserve] ALTER COLUMN [CustomerId] [int] NOT NULL
CREATE INDEX [IX_BusinessItemId] ON [dbo].[StaffBusinessItem]([BusinessItemId])
CREATE INDEX [IX_StaffId] ON [dbo].[StaffBusinessItem]([StaffId])
CREATE INDEX [IX_StaffId] ON [dbo].[CustomerReserve]([StaffId])
CREATE INDEX [IX_CustomerId] ON [dbo].[CustomerReserve]([CustomerId])
ALTER TABLE [dbo].[StaffBusinessItem] ADD CONSTRAINT [FK_dbo.StaffBusinessItem_dbo.BusinessItem_BusinessItemId] FOREIGN KEY ([BusinessItemId]) REFERENCES [dbo].[BusinessItem] ([Id]) ON DELETE CASCADE
ALTER TABLE [dbo].[StaffBusinessItem] ADD CONSTRAINT [FK_dbo.StaffBusinessItem_dbo.StaffDetail_StaffId] FOREIGN KEY ([StaffId]) REFERENCES [dbo].[StaffDetail] ([Id]) ON DELETE CASCADE
ALTER TABLE [dbo].[CustomerReserve] ADD CONSTRAINT [FK_dbo.CustomerReserve_dbo.CustomerDetail_CustomerId] FOREIGN KEY ([CustomerId]) REFERENCES [dbo].[CustomerDetail] ([Id]) ON DELETE CASCADE
ALTER TABLE [dbo].[CustomerReserve] ADD CONSTRAINT [FK_dbo.CustomerReserve_dbo.StaffDetail_StaffId] FOREIGN KEY ([StaffId]) REFERENCES [dbo].[StaffDetail] ([Id]) ON DELETE CASCADE
DECLARE @var4 nvarchar(128)
SELECT @var4 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.StaffBusinessItem')
AND col_name(parent_object_id, parent_column_id) = 'StoreId';
IF @var4 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[StaffBusinessItem] DROP CONSTRAINT [' + @var4 + ']')
ALTER TABLE [dbo].[StaffBusinessItem] DROP COLUMN [StoreId]
DECLARE @var5 nvarchar(128)
SELECT @var5 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.CustomerReserve')
AND col_name(parent_object_id, parent_column_id) = 'StoreId';
IF @var5 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [' + @var5 + ']')
ALTER TABLE [dbo].[CustomerReserve] DROP COLUMN [StoreId]
DECLARE @var6 nvarchar(128)
SELECT @var6 = name
FROM sys.default_constraints
WHERE parent_object_id = object_id(N'dbo.CustomerReserve')
AND col_name(parent_object_id, parent_column_id) = 'StaffBusinessItemId';
IF @var6 IS NOT NULL
    EXECUTE('ALTER TABLE [dbo].[CustomerReserve] DROP CONSTRAINT [' + @var6 + ']')
ALTER TABLE [dbo].[CustomerReserve] DROP COLUMN [StaffBusinessItemId]
INSERT [dbo].[__MigrationHistory]([MigrationId], [ContextKey], [Model], [ProductVersion])
VALUES (N'202211281835450_createAllTable', N'BonnieYork.Migrations.Configuration',   , N'6.2.0-61023')

